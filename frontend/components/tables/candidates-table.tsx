    ...
    return (
        <div className={`p-4 md:p-6 rounded-xl md:rounded-2xl border ${currentTheme.cardBorder} ${currentTheme.cardBg} transition-all shadow-lg overflow-hidden`}>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                ...
                <button className={`w-full sm:w-auto px-4 py-2 md:px-5 md:py-3 rounded-lg ${currentTheme.buttonBg} font-medium text-sm md:text-base whitespace-nowrap`}>+ Novo Candidato</button>
            </div>
            <div className="overflow-x-auto -mx-4 md:mx-0">
                <div className="inline-block min-w-full align-middle">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead><tr>{['NOME', 'EMAIL', 'SETOR', 'STATUS', 'AÇÕES'].map((h)=><th key={h} className={`px-3 md:px-6 py-3 text-left text-xs font-medium ${currentTheme.mainText} uppercase tracking-wider opacity-60`}>{h}</th>)}</tr></thead>
