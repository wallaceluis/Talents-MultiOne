...
export default function VacanciesPage() {
  ...
  const handleDelete = async (vacancy: Vacancy) => {
    if (!confirm(`Deseja realmente excluir a vaga "${vacancy.title}"?`)) return;
    try {
    ...
    <div className="p-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-600 dark:text-gray-400">Nenhuma vaga encontrada</div>
        ) : (
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {filteredVacancies.map((vacancy) => (
                <tr key={vacancy.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  <td className="px-6 py-4">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        <div className="text-sm font-medium text-gray-900 dark:text-white">{vacancy.title}</div>
                        <div className="text-sm text-gray-500">{vacancy.company?.name}</div>
                      </div>
                    </div>
                  </td>
